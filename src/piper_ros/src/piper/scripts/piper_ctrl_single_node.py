...
def ensure_can_interface_visible(can_port):
    operstate_path = "/sys/class/net/{}/operstate".format(can_port)
    if not os.path.exists(operstate_path):
        raise RuntimeError(
            "CAN socket '{}' does not exist in current runtime namespace. "
            "If running in Docker/Rocker, start container with "
    ...
    try:
        ...
        rospy.logwarn("[CAN] failed to read %s: %s", operstate_path, exc)
        return
    if state != "up":
...
class C_PiperRosNode():
    ...
    def __init__(self) -> None:
        ...
        if rospy.has_param('~can_port'):
            ...
            exit(0)
        self.can_judge_flag = rospy.get_param("~can_judge_flag", False)
        rospy.loginfo("%s is %s", rospy.resolve_name('~can_judge_flag'), self.can_judge_flag)
        # 是否自动使能，默认不自动使能
        ...
        # 发布末端位姿时使用的坐标系名称
        self.base_frame = rospy.get_param("~base_frame", "{}_base".format(self.can_port))
        rospy.loginfo("%s is %s", rospy.resolve_name('~base_frame'), self.base_frame)
        # 是否有夹爪，默认为有
        ...
        # 创建piper类并打开can接口
        ensure_can_interface_visible(self.can_port)
        try:
            self.piper = C_PiperInterface(
                can_name=self.can_port,
                judge_flag=self.can_judge_flag,
            )
            self.piper.ConnectPort()
        except Exception as exc:
            rospy.logfatal("[CAN] failed to open '%s': %s", self.can_port, exc)
            rospy.logfatal(
                "[CAN] hint: for Docker/Rocker use --network=host --privileged "
                "and make sure the CAN interface exists inside /sys/class/net."
            )
            raise
    ...
    def PublishArmEndPose(self):
        ...
        endpos.header.frame_id = self.base_frame
