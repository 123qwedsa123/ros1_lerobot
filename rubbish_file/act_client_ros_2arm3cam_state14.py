...
class ACTClient3Cam:
    def __init__(self):
        rospy.init_node("act_client_ros_3cam_state14", anonymous=True)
        ...
        rospy.loginfo("=" * 60)
        rospy.loginfo("ACT Client ROS - 三相机 (state14)")
    ...
    def zmq_req(self, cmd, jpg_bytes1=b"", jpg_bytes2=b"", jpg_bytes3=b"", state14=None):
    ...
    def wait_ready(self):
        ...
        while not rospy.is_shutdown():
            ...
            if time.time() - t0 > 5.0:
                rospy.logwarn(f"等待超时: img1={self.got_img1} img2={self.got_img2} img3={self.got_img3} s1={self.got_s1} s2={self.got_s2}")
                t0 = time.time()
    ...
    def run_stream(self):
        ...
        while not rospy.is_shutdown():
            ...
            try:
                meta, payload = self.zmq_req("infer", jpg1, jpg2, jpg3, state14)
